...
public class AnimationTest extends BasicGame {
	...
	public void init(GameContainer container) throws SlickException {
		...
		*/
		...
		SpriteSheet sheet = new SpriteSheet("data/madscientist_spritesheet.png", 32, 64); //(sheet location, sprite width, sprite height)
		...
		limited.stopAt(2); //the frame number, starting from zero, set higher than number to infinitely loop
		...
		container.getGraphics().setBackground(new Color(0.2f,0.4f,0.2f));
		...
		for (int i = 0; i < 8; i++) {
			for (int j = 0; j < 3; j++) {
				walk.addFrame(sheet.getSprite(j, i), 250);
			}
			for (int j = 0; j < 3; j++) {
				walk.addFrame(sheet.getSprite(j, i), 250);
			}
